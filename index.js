...
const yk = new WAConnection()    
yk.version = [2, 2119, 6]
yk.logger.level = 'warn'
let authofile = './YakkoXCode.json'
yk.on('qr', () => {
...
fs.existsSync(authofile) && yk.loadAuthInfo(authofile)
yk.on('connecting', () => {
    const time_connecting = moment.tz('Asia/Jakarta').format('HH:mm:ss')
...
await yk.connect({ timeoutMs: 30 * 1000 })
fs.writeFileSync(authofile, JSON.stringify(yk.base64EncodedAuthInfo(), null, '\t'))

// CONFIG
selep = true
afk_respon = {}
var prefix = "."
var apikey = "1e957a742c13"
var fakethumb = '' || fs.readFileSync('./zenz/fake.jpeg')

yk.on('chat-update', async (mek) => {
	try {
		...
		const q = args.join(' ')
		const botNumber = yk.user.jid
		const isGroup = from.endsWith('@g.us')
        
        const arg = budy.slice(command.length + 2, budy.length)
		const sender = isGroup ? mek.participant : mek.key.remoteJid
		const totalchat = await yk.chats.all()
		const groupMetadata = isGroup ? await yk.groupMetadata(from) : ''
        ...
        const conts = mek.key.fromMe ? yk.user.jid : yk.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? yk.user.name : conts.notify || conts.vname || conts.name || '-'
        ...
        const reply = (teks) => {
            yk.sendMessage(from, teks, text, {quoted:mek})
        }

        const sendMess = (hehe, teks) => {
            yk.sendMessage(hehe, teks, text)
        }

        const mentions = (teks, memberr, id) => {
            (id == null || id == undefined || id == false) ? yk.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : yk.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
        ...
        if (!isGroup && !isCmd && isSimi) {
            if (mek.key.fromMe === false && ! from.includes("status@broadcast")){
                await yk.updatePresence(from, Presence.composing)
            try {
            ...
            if (!isGroup){
                if (mek.key.fromMe === false && ! from.includes("status@broadcast")){
                    if(! afk_respon[sender]){
                        ...
                        yk.sendMessage(from, menu.afk(pushname, getReason), text, [sender])
                    }
                }
            }
        }
        ...
        if (resbutton == 'Creepy Fact') {
            ...
            yk.sendMessage(from, buffer, image, { quoted: mek })
        ...
        switch (commandstik) {
            ...
            case "sy02F/UUYrbPVGOKfTuEakAK/KIfwmipVKZ2QS6pe4A=":
                let stikquotes = yk.prepareMessageFromContent(from, {
                    "listMessage":  {
                        "title": "*Y A K K O - X C O D E*",
            ...
            yk.relayWAMessage(stikquotes, {waitForAck: true})
            break
            case "7THhxG0GXfRzCR2YFXjBmOExfhf7ialY2vd9VFm6VGE=":
                let stikrandom = yk.prepareMessageFromContent(from, {
                    "listMessage":  {
                        "title": "*Y A K K O - X C O D E*",
            ...
            yk.relayWAMessage(stikrandom, {waitForAck: true})
            break
            /*case '7THhxG0GXfRzCR2YFXjBmOExfhf7ialY2vd9VFm6VGE=':
                reply(`Mengrandom`)
            break*/
        ...
        switch (command) {
            ...
            case 'hash':
                ...
                const mediastick = await yk.downloadMediaMessage(encmeds)
            ...
            case 'menu':
                ...
                var textnya = " Y A K K O - X C O D E"
                menus = {
                    contextInfo: {
                ...
                yk.sendMessage(from, menu.Help(prefix), text, menus)
            break
            case 'quotes':
                let quotes = yk.prepareMessageFromContent(from, {
                    "listMessage":  {
                        "title": "*Y A K K O - X C O D E*",
                        "description": "Available Quotes Commands",
                        "buttonText": "Open Here",
            ...
            yk.relayWAMessage(quotes, {waitForAck: true})
            break
            case 'random':
                let random = yk.prepareMessageFromContent(from, {
                    "listMessage":  {
                        "title": "*Y A K K O - X C O D E*",
                        "description": "Available Random Commands",
                        "buttonText": "Open Here",
            ...
            yk.relayWAMessage(random, {waitForAck: true})
            break
            ...
            if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedSticker) && args.length == 0) {
                ...
                setb = await yk.downloadMediaMessage(setf)
            ...
            case 'public':
                if (!mek.key.fromMe) return
                if (selep === false) return yk.sendMessage(from, 'Mode Public already active', text)
                selep = false
                reply(`Success activated Mode Public`)
            break
            case 'self':
                if (!mek.key.fromMe) return 
                if (selep === true) return yk.sendMessage(from, 'Mode Self already active', text)
                selep = true
                reply(`Success activated Mode Self`)
            break
            /*case 'simi':
                if (ar[0] === 'enable') {
                    if (simih === false) return yk.sendMessage(from, 'Mode Simi already active', text)
                    simih = false
                    reply(`Success activated Mode Simi`)
                } else if (ar[0] === 'disable') {
                    if (simih === true) return yk.sendMessage(from, 'Mode Simi already deactive', text)
                    simih = true
            ...
            case 'afk':
                ...
                fs.writeFileSync('./settings.json', JSON.stringify(setting, null, 2))
                yk.sendMessage(from, `Afk activated dengan alasan :\n${setting.responder.reason}`, text)
            break
            case 'unafk':
                if (setting.responder.status == false){
                    yk.sendMessage(from, `Afk Mode already deactivated`, text)
                } else {
                    setting.responder.status = false
                    fs.writeFileSync('./settings.json', JSON.stringify(setting, null, 2))
                    yk.sendMessage(from, `Afk deactivated`, text)
                }
            break
            ...
            default:
                ...
                if (budy.startsWith('>')){
                    try {
                        return yk.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
                    } catch(err) {
                        e = String(err)
                        reply(e)
                    }
